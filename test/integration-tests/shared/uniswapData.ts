...
import { DEFAULT_FORK_BLOCK, FORGE_ROUTER_ADDRESS, TEST_RECIPIENT_ADDRESS } from './mainnetForkHelpers'
...
export async function getUniswapPools(forkBlock?: number): Promise<UniswapPools> {
  const fork = forkBlock ?? DEFAULT_FORK_BLOCK
...
function getProvider(): ethers.providers.BaseProvider {
  return new ethers.providers.JsonRpcProvider(process.env['FOUNDRY_RPC_URL'])
}
